                    ...
                    <table className="w-full text-xs">
                        <thead>
                            <tr style={{ background: 'var(--bg-card)', borderBottom: '1px solid var(--border)' }}>
                                ...
                                {bodegas.map((b, i) => (
                                    <th key={b.id}
                                        className="text-right px-1 py-2 font-medium text-[10px] uppercase tracking-wider whitespace-nowrap"
                                        style={{ color: 'var(--text-muted)', ...(i === 0 ? { borderLeft: '1px solid var(--border)' } : {}) }}>
                        ...
                        <tbody>
                            {listas.data.length === 0 ? (
                                <tr>
                                    <td colSpan={7 + bodegas.length} className="text-center py-16" style={{ color: 'var(--text-muted)' }}>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {bodegas.map((b, i) => (
                                            <td key={b.id}
                                                className="px-1 py-1.5 whitespace-nowrap text-[11px] text-right font-mono"
                                                style={{ color: 'var(--text-muted)', ...(i === 0 ? { borderLeft: '1px solid var(--border)' } : {}) }}>
                                                {Number(row.inventario?.[b.id] ?? 0).toFixed(0)}
                                            </td>
                                        ))}
                                        {promoCell(row)}
                                    </tr>
                                    {promoOpen && (
                                        <tr style={{ borderColor: 'var(--border)' }}>
                                            <td colSpan={7 + bodegas.length} className="px-4 py-3" style={{ background: 'var(--bg-card)', borderTop: '1px dashed var(--border)' }}>

...
export default function Index() {
    ...
    const limpiarFiltros = () => {
        const limpio: Filtros = { cliente: '', fecha_desde: '', fecha_hasta: '', estado: '' }
        setFiltro(limpio)
        router.get(route('ventas.notas-credito.index'), {}, { preserveState: false })
    }
    ...
    return (
        <AppLayout>
            ...
            <PageHeader
                ...
                description="Gestión de notas de crédito emitidas"
                breadcrumbs={[{ label: 'Ventas' }, { label: 'Notas de Crédito' }]}
                actions={
                    <span className="text-sm" style={{ color: 'var(--text-muted)' }}>
                        {notas.total} nota{notas.total === 1 ? '' : 's'}
                    </span>
                }
            />
